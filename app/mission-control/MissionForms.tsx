...
export default function MissionForms() {
  ...
  return (
    <section className="space-y-10">
      ...
      {/* Corporals */}
      <div id="apply-corporal" className="border rounded-2xl p-5">
        <div className="flex items-center justify-between">
          <div className="font-semibold">Apply to become a Corporal</div>
          <button
            onClick={() => setOpen(open === 'CORPORAL' ? null : 'CORPORAL')}
            className="text-sm rounded-xl border px-3 py-2 hover:bg-gray-50"
          >
            {open === 'CORPORAL' ? 'Close' : 'Open form'}
          </button>
        </div>
        {open === 'CORPORAL' && <CorporalForm />}
      </div>
...
function CorporalForm() {
  return (
    <form method="POST" action="/mission-control/corporal/apply" className="mt-4 space-y-3">
      <label className="block">
        <span className="text-sm">Name</span>
        <input name="name" required className="mt-1 w-full border rounded-xl px-3 py-2" />
      </label>
      ...
      <label className="block">
        <span className="text-sm">Responsibilities (optional)</span>
        <textarea name="responsibilities" className="mt-1 w-full border rounded-xl px-3 py-2" rows={3} />
      </label>
